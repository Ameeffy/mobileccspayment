...
const AddPaymentMobile = () => {
  ...
  const [organization, setOrganization] = useState<{ name: string, photo: string | null } | null>(null);
...
//ngrok http --domain=utterly-stable-arachnid.ngrok-free.app 5000
//https://finalccspayment.onrender.com/ 
  ...
  const handlePaymentAction = async () => {
    ...
    try {
      ...
      const response = await fetch('https://finalccspayment.onrender.com/api/auth/add-transaction', {
        method: 'POST',
        headers: {
  ...
  <TouchableOpacity
          onPress={() => {
            Alert.alert('Proceed to Promissory Notes', 'Are you sure you want to proceed?', [
              { text: 'No', style: 'cancel' },
              { text: 'Yes', onPress: () => router.push(`/Addpromissorynotesscreen?paymentId=${paymentId}`) },
            ]);
          }}
          style={styles.promissoryButton}>
...
{organization && (
        <View style={styles.organizationContainer}>
          <Image
            source={organization.photo ? { uri: organization.photo } : require('../assets/images/ccsdeptlogo.png')}
            style={styles.logo}
          />
          <Text style={styles.organizationName} numberOfLines={1} ellipsizeMode="tail">{organization.name}</Text>
        </View>
      )}
      {infoModalVisible && (
  <View style={styles.infoModal}>
    <View style={styles.infoModalContent}>
      ...
      Please scan the QR code to pay the exact amount of fees. After payment, upload your proof of payment. Thank you!
      </Text>
      <TouchableOpacity
      ...
      {paymentQrcode && <Image source={{ uri: paymentQrcode }} style={styles.qrcodeImage} />}
      <Text style={styles.title}>{paymentName}</Text>
      <Text style={styles.title}>Amount: {paymentPrice} PHP</Text>
...
<TouchableOpacity
        ...
        disabled={loading}
      >
        {loading ? <ActivityIndicator color="white" /> : <Text style={styles.submitButtonText}>Submit Payment</Text>}
      </TouchableOpacity>
...
const styles = StyleSheet.create({
  ...
  organizationContainer: { flexDirection: 'row', alignItems: 'center', marginBottom: 20 },
  logo: {
    width: 60,
    height: 60,
    borderRadius: 30,
  ...
  organizationName: { fontSize: 16, fontWeight: 'bold', color: 'white', flexShrink: 1 },
  buttonContainer: {
    flexDirection: 'row',
